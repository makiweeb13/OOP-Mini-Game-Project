...
public class WinnerState {
    ...
    public WinnerState(Pokemon winner) {
        this.winner = winner;
        lastToggleTime = System.currentTimeMillis();
    }
    ...
    public void render(Graphics g, int width, int height) {
        try {
            if (backgroundImage == null) {
                backgroundImage = ImageIO.read(new File("resources/images/winner.jpg"));
            }
        } catch(IOException e) {
            e.printStackTrace();
        }
        ...
        g.drawString(message, width/2 - 200, height/2);
        ...
        if (showText) {
            ...
            g.drawString("Press Enter to play again", width/2 - 230, height/2 + 100);
        }
    }
